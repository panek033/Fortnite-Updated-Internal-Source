...
enum offsets : std::int32_t {
	bonematrix = 0x1515F44,
	staticfindobject = 0x13EFEC8,
	drawtransition = 112,
	processevent = 76,
	offset_get_name = 0x18,
	offset_displayname = 0x90,
	offset_tier = 0x73,
	offset_viewportclient = 0x78,
	LastFireTime = 0xc98,
	CustomTimeDilation = 0x64,
	LastFireTimeVerified = 0xc9c,
	offset_get_font = 0x70,
	offset_get_world = 0x78,
	offset_balreadysearched = 0xD92,
	offset_clip_x = 0x30,
	WeaponData = 0x4A8,
	offset_clip_y = 0x34,
	offset_get_item_definition = 0x350 + 0x18,
	offset_is_active = 0x8A, //0x2b8
	offset_get_pawn_mesh = 0x318,
	CurrentVehicle = 0x2840,
	offset_input_pitch_scale = 0x53C,
	GlobalAnimRateScale = 0xA68,
	bAdsWhileNotOnGround = 0x4AE0,
	bIsReloadingWeapon = 0x360,
	offset_input_yaw_scale = 0x538,
	offset_get_viewport = 0x78,
	offset_get_local_players = 0x38,
...
public:
	...
	static uobject* static_find_objecttwo(uobject* klass, uobject* outer, const wchar_t* name) {
		return reinterpret_cast<uobject * (*)(uobject*, uobject*, const wchar_t*)>(fn + offsets::staticfindobject)(klass, outer, name);
	}

	static uobject* static_find_object(uobject* klass, uobject* outer, const wchar_t* name, bool exact) {

		return reinterpret_cast<uobject * (*)(uobject*, uobject*, const wchar_t*, bool)>(fn + offsets::staticfindobject)(klass, outer, name, exact);
	}
...
public:
	...
	fvector get_muzzle_location(weapon* weapon) {


		struct { int32_t PatternIndex; fvector return_value; } params;
		params.PatternIndex = (int32_t)weapon;
...
struct UMaterialInterface : uobject
{
	...
	UMaterial* GetMaterial() {
		...
		static uobject* function;
		if (!function) function = uobject::static_find_object(nullptr, nullptr, (L"MaterialInterface.GetBaseMaterial"), false);
		this->process_event(function, &params);
		return params.return_value;
	}
...
{
	//	float K2_GetScalarParameterValue(struct FName ParameterName); // Function Engine.MaterialInstanceDynamic.K2_GetScalarParameterValue // (Final|Native|Public|BlueprintCallable) // @ game+0x219d280
	float K2_GetScalarParameterValue(fname ParameterName)
	{
		uobject* Function = uobject::static_find_object(0, 0, (L"MaterialInstanceDynamic.K2_GetScalarParameterValue"), false);
...
public:
	...
	fvector get_bone_location(std::int32_t index) {
		fmatrix out_matrix = { };
		reinterpret_cast<fmatrix* (*)(mesh*, fmatrix*, std::int32_t)>(fn+ offsets::bonematrix)(this, &out_matrix, index);
...
public:
	...
	BOOL ClientSetRotation(const fvector& NewRotation, bool bResetCamera)
	{
		...
		{
			fvector										NewRotation;                                              // (Parm, ZeroConstructor, IsPlainOldData)
		...
		static uobject* function;
		if (!function) function = uobject::static_find_object(nullptr, nullptr, (L"Engine.Controller.ClientSetRotation"), false);
		this->process_event(function, &params);
		return TRUE;
	}
...
namespace defines {
	...
	static uobject* mapmarkertest = nullptr;
	...
	static bool init() {
		...
		defines::mapmarkertest = uobject::static_find_object(nullptr, nullptr, (L"AthenaPlayerMarker_WithCustomization"), false);
		...
		defines::LShift = fkey{ defines::string->string_to_name(L"LeftShift"), 0 };
		...
		defines::caps_lock = fkey{ defines::string->string_to_name(L"CapsLock"), 0 };
